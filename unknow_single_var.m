...
% Metadata
K = 3;    % Number of arms
T = 1e6;   % K=100, T=1e5, M=70 => ~ 1 minute.    K=100, T=1e6, M=70 => ~10 minutes.
M = 1000;   %1000 Number of MC samples
Nexp = 100; % Number of episodes to average the regret out
...
for i = 1:Nexp
    ...
    mhat_re_ts = zeros(K,1);
    mhat_im_ts = mhat_re_ts;
    ...
    tt = 0;
        ...
        for t = 1:T
            ...
            % UCB
                ...
                % Udapte p    
                b = muhat_re.^2 + muhat_im.^2 + sqrt(.5*vhat*log(t+1)./sp);
                [~,ind] = max(b);
                p = zeros(K,1);
                p(ind) = 1;
...
%                 end
            ...
            % TS
                m_re_post_ts = lambda^2*spw_re_ts ./ (vhat_ts + lambda^2*sp_ts); % Re( post_mean )
                m_im_post_ts = lambda^2*spw_im_ts ./ (vhat_ts + lambda^2*sp_ts); % Im( post_mean )
                v_post_ts = lambda^2 ./ (1 + lambda^2*sp_ts/vhat_ts);   % Posterior variance of all arms
                ...
                % Update statistics
                mhat_re_ts(ind_ts) = (mhat_re_ts(ind_ts)*sp_ts(ind_ts) + p_ts'*X_re)/(sp_ts(ind_ts)+1);
                mhat_im_ts(ind_ts) = (mhat_im_ts(ind_ts)*sp_ts(ind_ts) + p_ts'*X_im)/(sp_ts(ind_ts)+1);
                sp_ts  = sp_ts + p_ts;
                spw_re_ts = spw_re_ts + p_ts.*X_re;
                spw_im_ts = spw_im_ts + p_ts.*X_im;
                for k = 1:K
                    svhat_ts = svhat_ts + (sqrt(2*p_ts(k))*(X_re(k) - mhat_re_ts(k)))^2; % (sqrt(2*p_ts(k))*(X_re(k) - m_re_post_ts(k)))^2;
                    svhat_ts = svhat_ts + (sqrt(2*p_ts(k))*(X_im(k) - mhat_im_ts(k)))^2; % (sqrt(2*p_ts(k))*(X_im(k) - m_im_post_ts(k)))^2;
                end
            ...
            % WTS
%                 m_re_post_wts = lambda^2*spw_re_wts ./ (vhat_wts + lambda^2*sp_wts); % Re( post_mean )
%                 m_im_post_wts = lambda^2*spw_im_wts ./ (vhat_wts + lambda^2*sp_wts); % Im( post_mean )
%                 v_post_wts = lambda^2 ./ (1 + lambda^2*sp_wts/vhat_wts);   % Posterior variance of all arms
% 
%                 % Reward generation
...
%                 end
%                 vhat_wts = svhat_wts/(2*t*K);
% 
%                 % Expected Cumulative Regret computation
%                 R_wts(t) = R_wts(max(t-1,1)) + max(0,maxmu^2 - p_wts'*(abs(G).^2));
%                 instant_R(t) = maxmu^2-p_wts'*(abs(G).^2);
%                 
%                 % Udapte p    
%                 samples_re_wts = diag(sqrt(v_post_wts))*randn(K,M) + m_re_post_wts*ones(1,M);
...
for k = 1:K
    if k ~= index_maxmu
        single_constant = single_constant + (maxmu^2-abs(G(k))^2)/(abs(G(index_maxmu)-G(k))^2);
    end
end
single_constant = single_constant*sigma^2;
time = 1:T;
...
for k = 1:K
    if k ~= index_maxmu
        mult_constant = mult_constant + (maxmu^2-abs(G(k))^2)/(log(1 + abs(G(index_maxmu)-G(k))^2/sigma^2));
    end
end
plot(time,mult_constant*log(time),'g')


legend('UCB-V', 'TS', 'WTS', '\sigma^2/\Delta', '\Delta(log(1+\Delta/\sigma^2))^{-1}');
set(gca, 'XScale', 'log')
